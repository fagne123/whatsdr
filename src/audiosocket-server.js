...
class AudioSocketServer extends EventEmitter {
  ...
  /**
   * Envia comando HANGUP para o Asterisk encerrar a chamada
   * Protocolo AudioSocket: HANGUP = [0x00, 0x00, 0x00]
   */
  sendHangup(sessionId) {
    const session = this.sessions.get(sessionId);
    if (!session || !session.socket) {
      console.log('⚠️  Sessão não encontrada para hangup:', sessionId);
      return;
    }

    console.log('📞 Enviando comando HANGUP para Asterisk...');

    // Protocolo AudioSocket: HANGUP = [kind=0x00, size_hi=0x00, size_lo=0x00]
    const hangupMessage = Buffer.from([KIND_HANGUP, 0x00, 0x00]);

    try {
      session.socket.write(hangupMessage);
      console.log('✅ Comando HANGUP enviado');
    } catch (error) {
      console.error('❌ Erro ao enviar HANGUP:', error.message);
    }
  }

  /**
   * Encerra a sessão enviando HANGUP e limpando recursos
   */
  endSession(sessionId) {
    this.sendHangup(sessionId);
    this.endCall(sessionId);
  }
